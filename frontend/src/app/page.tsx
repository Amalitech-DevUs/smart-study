          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {subjects.map((subj, i) => (
              <ScrollReveal key={subj.slug} delay={i * 0.05}>
                <div
                  className={`flex flex-col justify-between rounded-xl border border-slate-200 border-l-4 ${subj.accent} bg-white p-6 shadow-sm`}
                >
                  <div>
                    <div className="flex items-start justify-between gap-2">
                      <h3 className="font-heading text-base font-bold text-slate-900">{subj.name}</h3>
                      <span className={`shrink-0 rounded-md px-2 py-0.5 text-[11px] font-semibold ${subj.badge}`}>
                        {subj.year}
                      </span>
                    </div>
